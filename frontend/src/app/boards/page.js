      ...
      <main className="px-8 py-10">
        ...
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-48 rounded-lg bg-surface border border-border animate-pulse" />
            ))}
          </div>
        )}
        ...
        {!loading && boards.length === 0 && (
          <div className="text-center py-20 px-6 rounded-lg border border-dashed border-border">
            <p className="text-xs font-mono uppercase tracking-widest text-accent mb-3">Empty</p>
            <h3 className="text-lg font-medium tracking-tight mb-2">No boards yet</h3>
            <p className="text-sm text-text-muted mb-6 max-w-sm mx-auto">
              Create your first board to start organizing tasks. Each board is a kanban for one project.
            </p>
            <button
              onClick={() => document.querySelector('input[placeholder="New board name"]')?.focus()}
            ...
            {boards.map((board, i) => {
              ...
              return (
                <div
                  ...
                  className={`
                    relative group rounded-lg border bg-surface flex flex-col transition-all
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    <div>
                      {isEditing ? (
                        ...
                        <h3 className="text-base font-medium tracking-tight pr-7 truncate">{board.name}</h3>
                      )}
                    </div>
                    ...
                    {columnCount > 0 && (
                      <div className="flex gap-1 mt-3">
                        {[...Array(Math.min(columnCount, 6))].map((_, idx) => (
                          <div key={idx} className="flex-1 h-8 rounded-sm bg-bg border border-border" />
                        ))}
                        {columnCount > 6 && (
          ...
          <div className="w-full max-w-sm rounded-lg bg-surface border border-border shadow-2xl">
            <div className="p-6 space-y-3">
              ...
              <p className="text-sm text-text-muted">
                ...
                <span className="text-text font-medium">{confirmDelete.name}</span> will be permanently deleted. This cannot be undone.
              </p>
            </div>
